...
/*
static PyObject *_spectrogram_yhh(PyObject *self, PyObject *args, PyObject *kwargs){
    ...
    static char *kwlist[] = {"input_data", "fs", "N", "overlap_percent", "win", NULL};
    if (!PyArg_ParseTupleAndKeywords(args, kwargs, "i|ii", kwlist, &input_data, &fs, &N, &overlap_percent, &win)) 
    {
        return NULL;
    }

    result = spectrogram_yhh(input_data, fs, N, overlap_percent, win);
    
    const Py_ssize_t tuple_i = result.size();
    const Py_ssize_t tuple_j = result[0].size();
    PyObject *list = PyList_New(0);
    for (unsigned i=0; i<result.size(); i++){
        PyObject *tuple = PyTuple_New(tuple_j);
        for (Py_ssize_t j=0; j<tuple_j; j++){
            PyObject *object = PyFloat_FromDouble(result[i][j]);
            PyTuple_SET_ITEM(tuple, j, object);
        }
        PyList_Append(list, tuple);
    }
    return Py_BuildValue("i", list);
}

static PyMethodDef SpeedupPerformanceMethods[] = {
  {"spectrogram_yhh", (PyCFunction) _spectrogram_yhh, METH_VARARGS | METH_KEYWORDS, "A spectrogram writen by yhh"},
